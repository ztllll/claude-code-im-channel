...
claude's ``rate_limit_event`` (in stream-json output) only carries the
*currently active* window — usually just ``five_hour``. To show the user
both their 5-hour rolling window AND their 7-day weekly window in /status,
we hit the same endpoint claude TUI uses, with the OAuth bearer token
sitting in ``~/.claude/.credentials.json``.
...
On sub2api/OneAPI relay deployments the token is either absent or rejected;
:func:`fetch_quota` returns ``None`` and ``/status`` shows a "中转端点
不支持" fallback.
"""
...
def _do_fetch() -> tuple[dict | None, str | None]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "im-claude-channel/quota.py",
...
def fetch_quota(*, force: bool = False) -> tuple[dict | None, float, str | None]:
    ...
    ``fetched_at`` is the unix time of the last *successful* fetch
    (0 if never succeeded). ``last_error`` carries the most recent
    failure reason, useful for showing "(failed: ...)" in /status.
    """
